function [of, ofa]=Lasso_forecasts(D, variables_to_forecast, sample,tf, weight, set_type, weight_ave, a_differencing)
     ...
     for h = 1:length(weight)
       ...
       if strcmp(cn{1},'deterministics')
        ...
        % set the conditioning and active sets based on set_type
            ...
            if set_type==1
                Zind=conditioningset1(vcode, codes); % AR+const  conditioningset1 conditioningset1_Lasso_alt_nocs
...
ofa{h}.Rsquared=NaN(size(of{h}.npred));
 ...
 % report variables used in the forecasting regression
    ...
    of{h}.SVcell=SVcell;
    of{h}.method_name=['Lasso, expanding window, using lamnda=',num2str(w)];
