function save_sample_results(savefile,savesheet, D, Forecast_errors, Forecast_values, variables_to_save, Nopredictors, samplef, No_methods, Rsq, SVs, Method_names, horizon)
 ...
 if report_variable_selection 
   ...
   selected_methods_to_report([1,5],1)=true; % AR(1), all lambdas
   selected_methods_to_report(78,1)=true; % OCMT, active set 2, no downweighting at selection
   %selected_methods_to_report(15:20,1)=true; % OCMTb - sel downweighting, all lambdas
   selected_methods_to_report(134,1)=true; % Lasso, all lambdas
      ...
      for j=1:sum(ind)
          ...
          for k=1:sum(selected_methods_to_report)
            ...
            %  ['saving method',smnames{k,1}]
              ...
              SelV(2,4:Te+3)=D.time_periods_names(samplef,1)';
              % set of variables
...
%                   end
 ...
 for i=1:size(variables_to_save,2) 
   ...
   for m=1:No_methods
       ...
       for j=1:no_ac
           ...
           cp=cp+NoGroups;
            ...
            Apt(m,cp)=Stats{m}.pt(1,j+NoGroups);
   ...
   benchmark=1; % for country-speific DM tests, AR(1)
   cr='E780';
   xlswrite(savefile.main,squeeze(DM.country(:,benchmark,:)),vfsheet,cr);
   benchmark=78; % for country-speific DM tests
   cr='E1870';
   xlswrite(savefile.main,squeeze(DM.country(:,benchmark,:)),vfsheet,cr);
   benchmark=84; % for country-speific DM tests
   cr='E2026';
   xlswrite(savefile.main,squeeze(DM.country(:,benchmark,:)),vfsheet,cr);
   ...
   %PT tests
   cr='E2182';
   xlswrite(savefile.main,Apt,vfsheet,cr); 
