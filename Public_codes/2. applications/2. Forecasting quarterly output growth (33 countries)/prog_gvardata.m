% This code is for forecasting cross-country GDP growth empirical application presented in:
...
%% load data
data_loaded=0;
h_horizon=4; % forecasting horizon either h=1 or h=4 or h=8
...
% boosting is reported as the  OCMTm1+7*Noweight : OCMTm1+8*Noweight-1            
...
% boosting reported as the  OCMTm2+7*Noweight : OCMTm2+8*Noweight-1      
       
       [ofb]=Boosting_forecasts(D,variables_to_forecast, sample,tf, weight, set_type, weight_ave, a_differencing);
       for i=1:Noweight
            fcst(:,OCMTm2+7*Noweight+i-1)=ofb{i}.forecasts(tf,:)';
            npred(:,OCMTm2+7*Noweight+i-1)=ofb{i}.npred; 
            svs(OCMTm2+7*Noweight+i-1)={ofb{i}.SVcell};
            Method_names(OCMTm2+7*Noweight+i-1)={ofb{i}.method_name};
       end      
